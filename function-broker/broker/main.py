...
CAPABILITIES_FILE = Path(os.getenv("CAPABILITIES_FILE", Path(__file__).parent / "config" / "capabilities.yaml"))
...
class HTTPAdapter:
    ...
    async def call(
        ...
        try:
            ...
            logger.exception("Engine call failed: %s", capability.id)
            return NormalizedResult(
                data=None,
                success=False,
                error=ErrorDetail(
...
@app.post("/v1/invoke")
async def invoke(req: InvokeRequest) -> Dict[str, Any]:  # Returns NormalizedResult.to_transport_dict()
    """
    ...
    if not capability:
        logger.warning("Unknown capability requested: %s", req.capability_id)
        return NormalizedResult(
            data=None,
            success=False,
            error=ErrorDetail(
            ...
            traces={"requested_capability_id": req.capability_id},
        ).to_transport_dict()
    ...
    try:
        ...
        logger.warning("Invalid envelope: %s", type(e).__name__)
        return NormalizedResult(
            data=None,
            success=False,
            error=ErrorDetail(
                code="INVALID_ENVELOPE",
                message="Request envelope is malformed",
                category="client",
                retryable=False,
            ),
        ).to_transport_dict()
    ...
    logger.info(
        "Invoking %s (tenant=%s)",
        req.capability_id, envelope.tenant_id,
    )
    ...
    try:
        ...
        logger.exception("Dispatch failed for capability: %s", req.capability_id)
        return NormalizedResult(
            data=None,
            success=False,
            error=ErrorDetail(
                code="DISPATCH_ERROR",
                message="Failed to dispatch request to execution engine",
                category="server",
                retryable=True,
            ),
            traces={"capability_id": req.capability_id},
        ).to_transport_dict()
