...
async def submit(
    ...
    for step in route["plan"]:
        ...
        try:
            ...
            logger.exception("Broker dispatch failed for step %s", step_id)
            result = NormalizedResult(
                data=None,
                success=False,
                error=ErrorDetail(
                    code="BROKER_UNREACHABLE",
                    message="Failed to connect to the function broker",
                    category="server",
                    retryable=True,
                ),
                traces={"step_id": step_id, "capability_id": step_capability},
            )
